    ...
    **kwargs,
    ...
    for attempt in range(max_retries + 1):
        ...
        if resp.status_code == 429:
            ...
            try:
                wait = min(float(resp.headers.get("Retry-After", 0)) or backoff_base * (2 ** attempt), 300.0)
...
# ── Step 2: Fetch Salesforce data spaces ─────────────────────────────────────
def fetch_data_spaces(instance_url: str, access_token: str) -> list:
    t0 = time.monotonic()
    ...
    if resp.status_code == 200:
        ...
        if spaces:
            log.info("  Found %d data space(s): %s (%.1fs)", len(spaces), spaces, time.monotonic() - t0)
            return spaces
...
# ── Step 3: SOAP Metadata API Retrieve ───────────────────────────────────────
def _soap_post(instance_url: str, body: str, action: str) -> ET.Element:
...
def fetch_metadata(instance_url: str, access_token: str) -> bytes:
    t0 = time.monotonic()
    log.info("Step 3: Retrieving ObjectSourceTargetMap metadata from Salesforce")

    envelope = _RETRIEVE_ENVELOPE.format(
        session_id=escape(access_token),
        api_version=escape(SF_API_VERSION),
    )
    root = _soap_post(instance_url, envelope, action="retrieve")
    ...
    for attempt in range(METADATA_MAX_POLLS):
        ...
        try:
            status_root = _soap_post(instance_url, status_body, action="checkRetrieveStatus")
            consecutive_failures = 0
        ...
        if done_el is None:
            log.warning("  Poll %d: SOAP response missing <done> element — treating as not done", attempt + 1)
...
# ── Step 4: Parse DLO→DMO edges ──────────────────────────────────────────────
def parse_edges(zip_bytes: bytes, fallback_dataspace: str) -> list:
    """
...
# ── Step 4b: Validate edges against MC catalog ───────────────────────────────
def _fetch_mc_catalog(warehouse_uuid: str, key_id: str, key_secret: str) -> dict:
    """
    ...
    Returns dict of {table_name_lower → data_space} using the MC catalog dataset field.
    When the same table name appears in multiple data spaces, the value is a list.
    """
    catalog: dict = {}
    after = None
    page = 0
    ...
    while True:
        ...
        for edge in (result.get("edges") or []):
            ...
            # Prefer the dedicated dataset field; fall back to parsing fullTableId
        ...
        if not after:
            log.warning("  MC catalog hasNextPage=true but endCursor is null — stopping pagination to avoid infinite loop")
            break

    return catalog


def _resolve_catalog(catalog: dict, table_name: str) -> Optional[str]:
    """Look up table_name in the pre-fetched catalog dict. Returns data_space or None."""
...
def validate_edges(
    ...
    try:
        mc_catalog = _fetch_mc_catalog(dc_warehouse_uuid, key_id, key_secret)
    ...
    log.info("  Fetched %d table(s) from MC catalog (%.1fs)", len(mc_catalog), time.monotonic() - t0)

    unique_names = sorted({e["source"] for e in edges} | {e["target"] for e in edges})
    mc_dataspace_map: dict = {}
    missing_set: set = set()

    for name in unique_names:
        data_space = _resolve_catalog(mc_catalog, name)
        if data_space is not None:
            mc_dataspace_map[name] = data_space
            log.debug("  Found in MC catalog: %s (data space: %s)", name, data_space)
        else:
    ...
    log.info(
        ...
        len(mc_dataspace_map), len(missing_set), time.monotonic() - t0,
    )

    if missing_set:
        log.warning(
            "  Table(s) not in MC catalog: %s",
    ...
    mismatch_skips = 0
    for e in edges:
        src_space = mc_dataspace_map.get(e["source"])
        tgt_space = mc_dataspace_map.get(e["target"])
...
# ── Push helpers ──────────────────────────────────────────────────────────────
def _make_ingestion_service(key_id: str, key_secret: str) -> IngestionService:
    return IngestionService(mc_client=Client(session=Session(
        mcd_id=key_id,
...
# ── Step 5: Push DLO→DMO lineage ─────────────────────────────────────────────
def push_dlo_dmo_edges(
    edges: list,
    svc: IngestionService,
    dc_resource_uuid: str,
    run_id: str,
) -> list:
    t0 = time.monotonic()
    ...
    for i in range(0, len(events), INGEST_BATCH_SIZE):
        ...
        try:
            resp = _send_lineage_batch(
                svc,
                resource_uuid=dc_resource_uuid,
...
class SalesforceDataCloudService:
    ...
    def fetch_data_spaces(self) -> list:
        return fetch_data_spaces(self.instance_url, self.token)

    def fetch_metadata(self) -> bytes:
        return fetch_metadata(self.instance_url, self.token)

    def parse_edges(self, zip_bytes: bytes, fallback_dataspace: str) -> list:
        return parse_edges(zip_bytes, fallback_dataspace)
...
class SalesforceDataCloudLineageService:
    ...
    def validate_edges(self, edges: list) -> list:
        return validate_edges(edges, self.resource_uuid, self.gql_key_id, self.gql_key_secret)

    def push_edges(self, edges: list, run_id: str) -> list:
        svc = _make_ingestion_service(self.ingest_key_id, self.ingest_key_secret)
        return push_dlo_dmo_edges(edges, svc, self.resource_uuid, run_id)
...
def main() -> None:
    ...
    try:
        ...
        # Step 2
        data_spaces = sf_svc.fetch_data_spaces()
    ...
    log.info(
        ...
        run_id, elapsed,
        len(dlo_edges),
        ", ".join(all_invocation_ids) if all_invocation_ids else "none",
    )
