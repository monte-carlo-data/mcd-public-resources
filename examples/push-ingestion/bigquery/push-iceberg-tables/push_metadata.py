"""Push a metadata manifest to Monte Carlo via the push ingestion API.

Reads the JSON manifest produced by collect_metadata.py and sends it
to Monte Carlo using the pycarlo SDK's IngestionService.
...
Usage:
    ...
    # For dev environment:
    MCD_INGEST_ID=<key-id> MCD_INGEST_TOKEN=<key-token> MCD_RESOURCE_UUID=<uuid> \
        python3 push_metadata.py --dev --input-file metadata_output.json
"""
